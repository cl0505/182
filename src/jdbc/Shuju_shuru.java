	...
	public static void main(String[] args) throws Exception {
		...
		for (File file : files) {
			...
			if (file.getName().endsWith("xlsx")) {
				read_xlsx_and_write_td(file);
			} else {
				read_xls_and_write_td(file);
			}
		}
	}
	private static void read_xlsx_and_write_td(File file) throws Exception {
...
//3.创建PreparedStatement语句
   PreparedStatement ydy_yuju=lianjie.prepareStatement("update sheet1 set `左眼裸眼视力`=?,`右眼裸眼视力`=? where `学号`=?");
		...
		private static void read_xls_and_write_td(File file) throws Exception {
